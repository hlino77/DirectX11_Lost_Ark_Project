...
CMonster_Zombie_Server::CMonster_Zombie_Server(const CMonster_Server& rhs)
	: CMonster_Server(rhs)
{
}

HRESULT CMonster_Zombie_Server::Initialize_Prototype()
{
...
void CMonster_Zombie_Server::Set_SlowMotion(_bool bSlow)
{
}

void CMonster_Zombie_Server::OnCollisionEnter(const _uint iColLayer, CCollider* pOther)
{
}

void CMonster_Zombie_Server::OnCollisionStay(const _uint iColLayer, CCollider* pOther)
{
}

void CMonster_Zombie_Server::OnCollisionExit(const _uint iColLayer, CCollider* pOther)
{
}

HRESULT CMonster_Zombie_Server::Ready_Components()
{
	...
	///* For.Com_Model */
	wstring strComName = L"Prototype_Component_Model_Monster_Zombie";
...
HRESULT CMonster_Zombie_Server::Ready_BehaviourTree()
{
	...
	DecoratorDesc.eDecoratorType = CBT_Decorator::DecoratorType::IF;
	CBT_Decorator* pIfTwist = CCommon_BT_IF_Twist_Server::Create(&DecoratorDesc);//넉백인가
	...
	DecoratorDesc.eDecoratorType = CBT_Decorator::DecoratorType::IF;
	CBT_Decorator* pIfBound = CCommon_BT_IF_Bound_Server::Create(&DecoratorDesc);//바운드인가
	...
	CBT_Action* pBug = CCommon_BT_Bound_Server::Create(&ActionDesc);
	...
	CBT_Action* pFear = CCommon_BT_Bug_Server::Create(&ActionDesc);
	...
	CBT_Action* pShock = CCommon_BT_Fear_Server::Create(&ActionDesc);
	...
	CBT_Composite* pSequence_Maz = CBT_Composite::Create(&CompositeDesc);
	if (FAILED(pSequence_Maz->AddChild(pStun)))
		return E_FAIL;	
	if (FAILED(pSequence_Maz->AddChild(pShock)))
		return E_FAIL;
	if (FAILED(pSequence_Maz->AddChild(pFear)))
		return E_FAIL;
	if (FAILED(pSequence_Maz->AddChild(pEarthquake)))
		return E_FAIL;
	if (FAILED(pSequence_Maz->AddChild(pBug)))
		return E_FAIL;

	DecoratorDesc.eDecoratorType = CBT_Decorator::DecoratorType::IF;
	CBT_Decorator* pIfMaz = CCommon_BT_IF_Maz_Server::Create(&DecoratorDesc);//상태이상인가?
	if (FAILED(pIfMaz->AddChild(pSequence_Maz)))
		return E_FAIL;
...
{
	CMonster_Zombie_Server* pInstance = new CMonster_Zombie_Server(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		cout << "Failed To Cloned : CMonster_Zombie" << endl;
		Safe_Release(pInstance);
	}
...
void CMonster_Zombie_Server::Free()
{
	__super::Free();
