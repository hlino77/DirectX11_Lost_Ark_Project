...
void CValtan_BT_Attack_Attack19_Server::OnStart()
{
	__super::OnStart(0);
...
CBT_Node::BT_RETURN CValtan_BT_Attack_Attack19_Server::OnUpdate(const _float& fTimeDelta)
{
	...
	if (m_pGameObject->Get_ModelCom()->Get_CurrAnim() ==m_vecAnimDesc[3].iAnimIndex&& m_pGameObject->Get_ModelCom()->IsNext())
		m_pGameObject->Get_TransformCom()->Turn_Speed(m_pGameObject->Get_TransformCom()->Get_State(CTransform::STATE_UP),XMConvertToRadians(360.f) ,fTimeDelta);
...
CValtan_BT_Attack_Attack19_Server* CValtan_BT_Attack_Attack19_Server::Create(void* pArg)
{
	CValtan_BT_Attack_Attack19_Server* pInstance = new CValtan_BT_Attack_Attack19_Server;
...
void CValtan_BT_Attack_Attack19_Server::Free()
{
	__super::Free();
}
