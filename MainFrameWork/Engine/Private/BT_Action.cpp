...
void CBT_Action::Add_Sound(_int iAnimIndex, _int iSoundOnIndex, wstring strSoundTag, _uint iSoiundChannel, _int iAnimFrame, _bool bUseLoopChannel, _float fLoopVolume)
{
	if (bUseLoopChannel)
	{
		if (m_pGameObject->Get_ModelCom()->Get_CurrAnim() == m_vecAnimDesc[iAnimIndex].iAnimIndex && !m_bSoundOn[iSoundOnIndex])
		{
			...
			CGameInstance::GetInstance()->PlaySoundFile_LoopChannel(strSoundTag + L".wav", fLoopVolume);
		}
	}
