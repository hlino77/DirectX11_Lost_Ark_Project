...
{
	_int iAnimIndex = m_pPlayer->Get_ModelCom()->Get_Anim_Frame(m_Attack_2);
	...
	else if (true == m_IsSkillContinue && 31 <= iAnimIndex)
	{
		Vec3 vClickPos;
		if (true == m_pPlayer->Get_CellPickingPos(vClickPos))
			m_pPlayer->Set_TargetPos(vClickPos);
	...
	else if (true == m_IsAttackContinue && 31 <= iAnimIndex)
	{
		Vec3 vClickPos;
		if (true == m_pPlayer->Get_CellPickingPos(vClickPos))
			m_pPlayer->Set_TargetPos(vClickPos);
...
void CState_WDR_Iden_Attack_2::Tick_State_NoneControl(_float fTimeDelta)
{
	...
	if (m_AttackFrames[m_iAttackCnt] <= (_int)m_pPlayer->Get_ModelCom()->Get_Anim_Frame((_uint)m_Attack_2))
	{
		m_iAttackCnt++;
		Effect_Shot();
	}
