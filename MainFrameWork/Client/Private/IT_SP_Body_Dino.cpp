#include "stdafx.h"
#include "..\Public\IT_SP_Body_Dino.h"
#include "Player.h"

CIT_SP_Body_Dino::CIT_SP_Body_Dino(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CItem(pDevice, pContext)
{
}

CIT_SP_Body_Dino::CIT_SP_Body_Dino(const CIT_SP_Body_Dino& rhs)
	: CItem(rhs)
{
}

HRESULT CIT_SP_Body_Dino::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CIT_SP_Body_Dino::Initialize(void* pArg)
{
	__super::Initialize(pArg);

	m_iItemGrade = (_uint)GRADE::PURPLE;
	m_iItemType = (_uint)TYPE::EQUIP;

	m_strObjectTag = TEXT("IT_SP_Body_Dino");
	m_strItemName = TEXT("´ÙÀÌ³ë ¸öÅë");
	m_strItemDescript = TEXT("");

	m_iEquipType = (_uint)PART::BODY;

	m_vecUseEquipSlot.push_back((_uint)PART::BODY);
	m_vecUseEquipSlot.push_back((_uint)PART::SHOULDER);
	m_vecUseEquipSlot.push_back((_uint)PART::ARM);
	m_vecUseEquipSlot.push_back((_uint)PART::LEG);

	m_tStatChangeDesc.iHp = 500;

	return S_OK;
}

void CIT_SP_Body_Dino::Tick(_float fTimeDelta)
{
}

void CIT_SP_Body_Dino::LateTick(_float fTimeDelta)
{
}

HRESULT CIT_SP_Body_Dino::Render()
{
	return S_OK;
}

HRESULT CIT_SP_Body_Dino::Use_Item(CPlayer* pOwner)
{
	if (nullptr == pOwner)
		return E_FAIL;

	CPlayer::STATDESC tPcStat = pOwner->Get_PlayerStat_Desc();

	tPcStat.iMaxHp += m_tStatChangeDesc.iHp;
	tPcStat.iCurHp += m_tStatChangeDesc.iHp;

	pOwner->Set_PlayerStat_Desc(tPcStat);

	for (auto& i : m_vecUseEquipSlot)
	{
		if (nullptr != pOwner->Get_EquipItem(i))
		{
			pOwner->Get_EquipItem(i)->Disuse_Item(pOwner, false);
		}
		pOwner->Set_EquipItem(i, this);

		if (i == m_iEquipType)
		{
			pOwner->Set_ModelPart(i, m_pModelCom);
		}
		else
		{
			pOwner->Set_ModelPart(i, nullptr);
		}
	}
	Set_EquipWearing(true);
	return S_OK;
}

HRESULT CIT_SP_Body_Dino::Disuse_Item(CPlayer* pOwner, _bool bUseDefault)
{
	if (nullptr == pOwner)
		return E_FAIL;
	Set_EquipWearing(false);
	CPlayer::STATDESC tPcStat = pOwner->Get_PlayerStat_Desc();

	tPcStat.iMaxHp -= m_tStatChangeDesc.iHp;
	tPcStat.iCurHp -= m_tStatChangeDesc.iHp;

	pOwner->Set_PlayerStat_Desc(tPcStat);

	for (auto& i : m_vecUseEquipSlot)
	{
		pOwner->Set_ModelPart(i, pOwner->Get_DefaultPart(i));
		pOwner->Set_EquipItem(i, nullptr);
	}
	if (pOwner->Is_Control() && false == bUseDefault)
		pOwner->Add_Item(m_strObjectTag, this);
	return S_OK;
}

void CIT_SP_Body_Dino::Upgrade_Item()
{
	m_tStatChangeDesc.iHp += 100;
}

HRESULT CIT_SP_Body_Dino::Ready_Components()
{
	__super::Ready_Components();

	CModel::CHANGECOLOR pChangeColor;
	pChangeColor.vColor_R = Vec4(0.089888f * 0.679775f, 1.f * 0.679775f, 0.673617f * 0.679775f, 1.f);
	pChangeColor.vColor_G = Vec4(1.30214f, 1.12166f, 0.373086f, 1.f);
	pChangeColor.vColor_B = Vec4(2.11905f, 1.31934f, 0.344261f, 1.f);

	wstring strComName = L"Prototype_Component_Model_SP_Body_Dino";
	if (FAILED(__super::Add_Component(LEVEL_STATIC, strComName, TEXT("Com_Model"), (CComponent**)&m_pModelCom, &pChangeColor)))
		return E_FAIL;

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_Inventory_Artist_Dinyong_Body"),
		TEXT("Com_Texture"), (CComponent**)&m_pItemTextureCom)))
		return E_FAIL;

	return S_OK;
}

CIT_SP_Body_Dino* CIT_SP_Body_Dino::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CIT_SP_Body_Dino* pInstance = new CIT_SP_Body_Dino(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CIT_SP_Body_Dino");
		Safe_Release(pInstance);
	}

	return pInstance;
}

CGameObject* CIT_SP_Body_Dino::Clone(void* pArg)
{
	CIT_SP_Body_Dino* pInstance = new CIT_SP_Body_Dino(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CIT_SP_Body_Dino");
		Safe_Release(pInstance);
	}

	return pInstance;
}

void CIT_SP_Body_Dino::Free()
{
	__super::Free();

	Safe_Release(m_pModelCom);
	Safe_Release(m_pItemTextureCom);
}
