#include "stdafx.h"
#include "..\Public\State_MG_Attack_2.h"
...
CState_MG_Attack_2::CState_MG_Attack_2(const wstring& strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Bard* pOwner)
	: CState(strStateName, pMachine, pController), m_pPlayer(pOwner)
{
}

HRESULT CState_MG_Attack_2::Initialize()
{
	m_Attack_1 = m_pPlayer->Get_ModelCom()->Initailize_FindAnimation(L"att_battle_1_02", 1.2f);
	if (m_Attack_1 == -1)
		return E_FAIL;

	if (m_pPlayer->Is_Control())
		m_TickFunc = &CState_MG_Attack_2::Tick_State_Control;
	else
		m_TickFunc = &CState_MG_Attack_2::Tick_State_NoneControl;
...
void CState_MG_Attack_2::Enter_State()
{
	m_iAttackCnt = 0;
...
void CState_MG_Attack_2::Tick_State(_float fTimeDelta)
{
	m_TickFunc(*this, fTimeDelta);
}

void CState_MG_Attack_2::Exit_State()
{
	m_IsAttackContinue = false;
}

void CState_MG_Attack_2::Tick_State_Control(_float fTimeDelta)
{
...
void CState_MG_Attack_2::Tick_State_NoneControl(_float fTimeDelta)
{
	m_pPlayer->Follow_ServerPos(0.01f, 6.0f * fTimeDelta);
}

CState_MG_Attack_2* CState_MG_Attack_2::Create(wstring strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Bard* pOwner)
{
	CState_MG_Attack_2* pInstance = new CState_MG_Attack_2(strStateName, pMachine, pController, pOwner);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("Failed To Cloned : CState_MG_Attack_2");
