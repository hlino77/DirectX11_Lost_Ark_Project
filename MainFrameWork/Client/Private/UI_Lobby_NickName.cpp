...
{
	m_strFont = L"던파연마된칼날";
	m_strClassFont = TEXT("넥슨Lv1고딕Bold");
	Ready_TextBox();
...
void CUI_Lobby_NickName::Print_NickName()
{
	...
	if (nullptr != m_pClassNameWnd)
	{
		...
		Vec2 vMeasure = CGameInstance::GetInstance()->MeasureString(L"넥슨Lv1고딕Bold", m_strClassName);
		Vec2 vOrigin = vMeasure * 0.5f;
