...
HRESULT CUI_LoadingLabelBottom::Render()
{
    ...
    Vec2 vMeasure = CGameInstance::GetInstance()->MeasureString(L"´øÆÄ¿¬¸¶µÈÄ®³¯", m_strTip);
    Vec2 vOrigin = vMeasure * 0.5f;
    CGameInstance::GetInstance()->DrawFont(L"´øÆÄ¿¬¸¶µÈÄ®³¯", m_strTip, vPos, Vec4(214.f / 255.f, 174.f/255.f, 90.f / 255.f, 1.0f), 0.0f, vOrigin, vScale);

    vPos = Vec2(m_fX, m_fY + 15.f);
    vScale = Vec2(0.5f, 0.5f);
    vMeasure = CGameInstance::GetInstance()->MeasureString(L"ÇÑÄÄ°íµñ", m_strToolTips);
...
void CUI_LoadingLabelBottom::Set_ToolTip()
{
    ...
    {
    case 0:
        m_strToolTips = TEXT("°Ç½½¸µ¾î°¡ ¾Æ´Ï¶ó °Ç°ÉÀÌºó´Ù.");
        break;

    case 1:
        m_strToolTips = TEXT("µ·Ä¡½ºÆÔÀº ¹«ÀûÀÔ´Ï´Ù.");
        break;

    case 2:
        m_strToolTips = TEXT("·Î¾Æ°¡ ¶â±æ ÁÙ ¸ô¶ú½ººó´Ù.");
        break;

    case 3:
        m_strToolTips = TEXT("ÅøÆÁ ¹¹ÀûÁö");
        break;

    case 4:
        m_strToolTips = TEXT("¾µ°Ô¾ø´Ù");
        break;
    }
}
