...
CBT_Node::BT_RETURN CValtan_BT_Attack_TrippleCounterChop_3::OnUpdate(const _float& fTimeDelta)
{
	...
	{
		m_bShoot = false;	
		Add_Sound(L"Effect", L"WWISEDEFAULTBANK_S_MOB_G_VOLTAN2#396 (63667459)");
	...
	Add_Sound(0, 0, L"Effect", L"WWISEDEFAULTBANK_S_MOB_G_VOLTAN2#43 (1015057785)");
	...
	Add_Sound(2, 3, L"Effect", L"WWISEDEFAULTBANK_S_MOB_G_VOLTAN2#459 (119952812)");
...
void CValtan_BT_Attack_TrippleCounterChop_3::On_FirstAnimStart()
{
...
void CValtan_BT_Attack_TrippleCounterChop_3::Update_Effect()
{
	...
	if (false == m_bEffectStart && iAnimFrame0 >= 31 && m_vecAnimDesc[2].iAnimIndex == m_pGameObject->Get_ModelCom()->Get_CurrAnim())
	{
		...
		CServerSessionManager::GetInstance()->Get_Player()->Get_Camera()->Cam_Shake(0.2f, 100.0f, 0.5f, 10.0f);
		m_bEffectStart = true;
	}
}
