...
HRESULT CLevel_ChaosLevel3::Ready_Layer_SkyBox(const LAYER_TYPE eLayerType)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);

	CGameObject* pSkyDome = pGameInstance->Add_GameObject(LEVEL_CHAOS_3, _uint(eLayerType), TEXT("Prototype_GameObject_SkyDome"));
	if (nullptr == pSkyDome)
		return E_FAIL;
...
HRESULT CLevel_ChaosLevel3::Ready_Layer_UI(const LAYER_TYPE eLayerType)
{
	...
	pUI = pGameInstance->Add_GameObject(LEVEL_BERN, _uint(eLayerType), TEXT("CUI_ChaosDungeon_Clear"));
	if (nullptr == pUI)
		return E_FAIL;
	else
		CUI_Manager::GetInstance()->Add_UI(LEVEL_BERN, static_cast<CUI*>(pUI));
...
HRESULT CLevel_ChaosLevel3::Send_UserInfo()
{
	...
	else
	{
		...
		CNavigationMgr::GetInstance()->Find_FirstCell(LEVEL_CHAOS_3, pPlayer);
		pPlayer->Reset_Level();
	}
