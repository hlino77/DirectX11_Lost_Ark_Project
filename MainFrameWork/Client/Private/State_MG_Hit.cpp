...
CState_MG_Hit::CState_MG_Hit(const wstring& strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Bard* pOwner)
	: CState(strStateName, pMachine, pController), m_pPlayer(pOwner)
{
}

HRESULT CState_MG_Hit::Initialize()
{
	...
	if (m_pPlayer->Is_Control())
		m_TickFunc = &CState_MG_Hit::Tick_State_Control;
	else
		m_TickFunc = &CState_MG_Hit::Tick_State_NoneControl;

	return S_OK;
}

void CState_MG_Hit::Enter_State()
{
...
void CState_MG_Hit::Tick_State(_float fTimeDelta)
{
	m_TickFunc(*this, fTimeDelta);
}

void CState_MG_Hit::Exit_State()
{
	m_fTimeAcc = 0.f;
...
void CState_MG_Hit::Tick_State_NoneControl(_float fTimeDelta)
{
	if (m_iHit == m_iHit_Down)
	{
		if (true == m_pPlayer->Get_ModelCom()->Is_AnimationEnd(m_iHit))
...
void CState_MG_Hit::Hit_Down(_float fTimeDelta)
{
	if (true == m_pPlayer->Get_ModelCom()->Is_AnimationEnd(m_iHit))
	{
...
CState_MG_Hit* CState_MG_Hit::Create(wstring strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Bard* pOwner)
{
	CState_MG_Hit* pInstance = new CState_MG_Hit(strStateName, pMachine, pController, pOwner);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("Failed To Cloned : CState_MG_Hit");
