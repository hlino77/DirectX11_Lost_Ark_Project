#include "stdafx.h"
#include "..\Public\State_WR_HitEnd.h"
...
CState_WR_HitEnd::CState_WR_HitEnd(const wstring& strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Slayer* pOwner)
	: CState(strStateName, pMachine, pController), m_pPlayer(pOwner)
{
}

HRESULT CState_WR_HitEnd::Initialize()
{
	...
	if (m_pPlayer->Is_Control())
		m_TickFunc = &CState_WR_HitEnd::Tick_State_Control;
	else
		m_TickFunc = &CState_WR_HitEnd::Tick_State_NoneControl;

	return S_OK;
}

void CState_WR_HitEnd::Enter_State()
{
...
void CState_WR_HitEnd::Tick_State(_float fTimeDelta)
{
...
void CState_WR_HitEnd::Tick_State_NoneControl(_float fTimeDelta)
{
	if (true == m_pPlayer->Get_ModelCom()->Is_AnimationEnd(m_iHitEnd))
	{
		m_pPlayer->Set_AnimationSpeed(0.f);
...
CState_WR_HitEnd* CState_WR_HitEnd::Create(wstring strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Slayer* pOwner)
{
	CState_WR_HitEnd* pInstance = new CState_WR_HitEnd(strStateName, pMachine, pController, pOwner);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("Failed To Cloned : CState_WR_HitEnd");
