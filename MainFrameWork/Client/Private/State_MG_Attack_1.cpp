#include "stdafx.h"
#include "..\Public\State_MG_Attack_1.h"
...
CState_MG_Attack_1::CState_MG_Attack_1(const wstring& strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Bard* pOwner)
	: CState(strStateName, pMachine, pController), m_pPlayer(pOwner)
{
}

HRESULT CState_MG_Attack_1::Initialize()
{
	...
	if (m_pPlayer->Is_Control())
		m_TickFunc = &CState_MG_Attack_1::Tick_State_Control;
	else
		m_TickFunc = &CState_MG_Attack_1::Tick_State_NoneControl;


	/* 일반공격 프레임 */
	m_AttackFrames.push_back(13);
	m_AttackFrames.push_back(-1);

	return S_OK;
}

void CState_MG_Attack_1::Enter_State()
{
	m_iAttackCnt = 0;
...
void CState_MG_Attack_1::Tick_State(_float fTimeDelta)
{
	m_TickFunc(*this, fTimeDelta);
}

void CState_MG_Attack_1::Exit_State()
{
	m_IsAttackContinue = false;
}

void CState_MG_Attack_1::Tick_State_Control(_float fTimeDelta)
{
...
void CState_MG_Attack_1::Tick_State_NoneControl(_float fTimeDelta)
{
	m_pPlayer->Follow_ServerPos(0.01f, 6.0f * fTimeDelta);
}

CState_MG_Attack_1* CState_MG_Attack_1::Create(wstring strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Bard* pOwner)
{
	CState_MG_Attack_1* pInstance = new CState_MG_Attack_1(strStateName, pMachine, pController, pOwner);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("Failed To Cloned : CState_MG_Attack_1");
