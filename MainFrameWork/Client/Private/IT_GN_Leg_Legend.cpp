#include "stdafx.h"
#include "..\Public\IT_GN_Leg_Legend.h"
#include "Player.h"

CIT_GN_Leg_Legend::CIT_GN_Leg_Legend(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CItem(pDevice, pContext)
{
}

CIT_GN_Leg_Legend::CIT_GN_Leg_Legend(const CIT_GN_Leg_Legend& rhs)
	: CItem(rhs)
{
}

HRESULT CIT_GN_Leg_Legend::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CIT_GN_Leg_Legend::Initialize(void* pArg)
{
	...
	m_strObjectTag = TEXT("IT_GN_Leg_Legend");
...
void CIT_GN_Leg_Legend::Tick(_float fTimeDelta)
{
}

void CIT_GN_Leg_Legend::LateTick(_float fTimeDelta)
{
}

HRESULT CIT_GN_Leg_Legend::Render()
{
	return S_OK;
}

HRESULT CIT_GN_Leg_Legend::Use_Item(CPlayer* pOwner)
{
	if (nullptr == pOwner)
		return E_FAIL;
...
HRESULT CIT_GN_Leg_Legend::Disuse_Item(CPlayer* pOwner, _bool bUseDefault)
{
	if (nullptr == pOwner)
		return E_FAIL;
...
void CIT_GN_Leg_Legend::Upgrade_Item()
{
	m_tStatChangeDesc.iHp += 200;
}

HRESULT CIT_GN_Leg_Legend::Ready_Components()
{
	__super::Ready_Components();

	wstring strComName = L"Prototype_Component_Model_GN_Leg_Legend";
...
CIT_GN_Leg_Legend* CIT_GN_Leg_Legend::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CIT_GN_Leg_Legend* pInstance = new CIT_GN_Leg_Legend(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CIT_GN_Leg_Legend");
...
CGameObject* CIT_GN_Leg_Legend::Clone(void* pArg)
{
	CIT_GN_Leg_Legend* pInstance = new CIT_GN_Leg_Legend(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CIT_GN_Leg_Legend");
		Safe_Release(pInstance);
	}
