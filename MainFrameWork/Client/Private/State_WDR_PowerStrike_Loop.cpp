...
{
	_int iAnimIndex = m_pPlayer->Get_ModelCom()->Get_Anim_Frame(m_iPowerStrike_Loop);
...
void CState_WDR_PowerStrike_Loop::Tick_State_NoneControl(_float fTimeDelta)
{
	...
	if (m_SkillFrames[m_iSkillCnt] <= (_int)m_pPlayer->Get_ModelCom()->Get_Anim_Frame((_uint)m_iPowerStrike_Loop))
	{
		m_iSkillCnt++;
