#include "stdafx.h"
#include "..\Public\State_WDR_Hit_Common.h"
...
CState_WDR_Hit_Common::CState_WDR_Hit_Common(const wstring& strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Destroyer* pOwner)
	: CState(strStateName, pMachine, pController), m_pPlayer(pOwner)
{
}

HRESULT CState_WDR_Hit_Common::Initialize()
{
	...
	if (m_pPlayer->Is_Control())
		m_TickFunc = &CState_WDR_Hit_Common::Tick_State_Control;
	else
...
void CState_WDR_Hit_Common::Tick_State(_float fTimeDelta)
{
	m_TickFunc(*this, fTimeDelta);
}

void CState_WDR_Hit_Common::Exit_State()
{
	m_fTimeAcc = 0.f;
}

void CState_WDR_Hit_Common::Tick_State_Control(_float fTimeDelta)
{
...
void CState_WDR_Hit_Common::Tick_State_NoneControl(_float fTimeDelta)
{
	m_pPlayer->Follow_ServerPos(0.01f, 6.0f * fTimeDelta);
}

void CState_WDR_Hit_Common::Hit_Dmg(_float fTimeDelta)
{
...
CState_WDR_Hit_Common* CState_WDR_Hit_Common::Create(wstring strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Destroyer* pOwner)
{
	CState_WDR_Hit_Common* pInstance = new CState_WDR_Hit_Common(strStateName, pMachine, pController, pOwner);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("Failed To Cloned : CState_WDR_Hit_Common");
...
void CState_WDR_Hit_Common::Free()
{
	__super::Free();
}
