...
CBT_Node::BT_RETURN CValtan_BT_Attack_FistSmashExplosion::OnUpdate(_float fTimeDelta)
{
	...
	{
		m_bShoot = false;
		Add_Sound(0, 0, L"Effect", L"Valtan#106 (917948767)");
		...
		if (pSkill != nullptr)
		{
			...
			pSkill->Get_TransformCom()->Set_State(CTransform::STATE_POSITION, vPos);
			pSkill->Get_TransformCom()->LookAt_Dir(vLook);		
			static_cast<CSkill*>(pSkill)->Set_SoundTag(L"Valtan#112 (934806095).wav");
	...
	if (m_bLastAttack == false && m_bShoot == false)
	{
		...
		if (m_fLastAttackDelay <= 0.0f)
		{
			...
			if (pSkill != nullptr && m_pGameObject->Get_NearTarget() != nullptr)
			{
				...
				static_cast<CSkill*>(pSkill)->Set_SoundTag(L"Valtan#37 (464077464).wav");
			}

			m_bLastAttack = true;
		}
	}
	Add_Sound(0, 0, L"Effect", L"Valtan#307 (599260869)");
