...
CState_SP_Hit::CState_SP_Hit(const wstring& strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Doaga* pOwner)
	: CState(strStateName, pMachine, pController), m_pPlayer(pOwner)
{
}

HRESULT CState_SP_Hit::Initialize()
{
	...
	if (m_pPlayer->Is_Control())
		m_TickFunc = &CState_SP_Hit::Tick_State_Control;
	else
		m_TickFunc = &CState_SP_Hit::Tick_State_NoneControl;


	m_SoundFrames.push_back(SOUNDDESC(0, TEXT("Effect"), TEXT("SP_417.wav"), 0.3f)); //  Skill
...
void CState_SP_Hit::Tick_State(_float fTimeDelta)
{
	m_TickFunc(*this, fTimeDelta);
}

void CState_SP_Hit::Exit_State()
{
	m_fTimeAcc = 0.f;
...
void CState_SP_Hit::Tick_State_Control(_float fTimeDelta)
{
...
void CState_SP_Hit::Tick_State_NoneControl(_float fTimeDelta)
{
	if (m_iHit == m_iHit_Down)
	{
		if (true == m_pPlayer->Get_ModelCom()->Is_AnimationEnd(m_iHit))
...
void CState_SP_Hit::Hit_Down(_float fTimeDelta)
{
	if (true == m_pPlayer->Get_ModelCom()->Is_AnimationEnd(m_iHit))
	{
...
CState_SP_Hit* CState_SP_Hit::Create(wstring strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Doaga* pOwner)
{
	CState_SP_Hit* pInstance = new CState_SP_Hit(strStateName, pMachine, pController, pOwner);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("Failed To Cloned : CState_SP_Hit");
