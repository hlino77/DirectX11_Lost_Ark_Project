...
HRESULT CLevel_ChaosLevel1::Initialize()
{
...
HRESULT CLevel_ChaosLevel1::Tick(const _float& fTimeDelta)
{
	/*if(KEY_TAP(KEY::F9))
...
HRESULT CLevel_ChaosLevel1::Ready_Lights()
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChaosLevel1::Ready_Layer_SkyBox(const LAYER_TYPE eLayerType)
{
	...
	desc.fAlpha = 0.4f;
	CGameObject* pSkyFloor = pGameInstance->Add_GameObject(LEVEL_CHAOS_1, _uint(eLayerType), TEXT("Prototype_GameObject_SkyFloor"), &desc);
	if (nullptr == pSkyFloor)
		return E_FAIL;

	pSkyFloor->Get_TransformCom()->Set_Scale(Vec3(140.f, 140.f, 140.f));
	pSkyFloor->Get_TransformCom()->Set_State(CTransform::STATE_POSITION, Vec3(100.f, -208.f, 100.f));

	// 두 번째
	desc.vTileCount = Vec2(4.f, 4.f);
	desc.vFloorUVoffset = Vec2(0.0043f, 0.0043f);
	desc.fAlpha = 0.4f;
	pSkyFloor = pGameInstance->Add_GameObject(LEVEL_CHAOS_1, _uint(eLayerType), TEXT("Prototype_GameObject_SkyFloor"), &desc);
	if (nullptr == pSkyFloor)
		return E_FAIL;
...
HRESULT CLevel_ChaosLevel1::Ready_Layer_Camera(const LAYER_TYPE eLayerType)
{
...
HRESULT CLevel_ChaosLevel1::Ready_Layer_Player(const LAYER_TYPE eLayerType)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChaosLevel1::Ready_Layer_BackGround(const LAYER_TYPE eLayerType)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChaosLevel1::Ready_Layer_Monster(const LAYER_TYPE eLayerType)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChaosLevel1::Ready_Layer_UI(const LAYER_TYPE eLayerType)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChaosLevel1::Ready_Layer_Effect(const LAYER_TYPE eLayerType)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChaosLevel1::Ready_Player_Camera(const LAYER_TYPE eLayerType)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	...
	CGameObject* pCamera = pGameInstance->Add_GameObject(LEVEL_CHAOS_1, _uint(eLayerType), TEXT("Prototype_GameObject_Camera_Player"), &CameraDesc);
	if (pCamera == nullptr)
		return E_FAIL;
...
HRESULT CLevel_ChaosLevel1::Send_UserInfo()
{
	...
	else
	{
		...
		pkt.set_inextlevel(LEVELID::LEVEL_CHAOS_1);

		SendBufferRef pSendBuffer = CClientPacketHandler::MakeSendBuffer(pkt);
		CServerSessionManager::GetInstance()->Send(pSendBuffer);

		pPlayer->Set_CurrLevel(LEVELID::LEVEL_CHAOS_1);
...
void CLevel_ChaosLevel1::Wait_ServerLevelState(LEVELSTATE eState)
{
...
HRESULT CLevel_ChaosLevel1::Load_MapData(LEVELID eLevel, const wstring& szFullPath)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChaosLevel1::Ready_Events()
{

	return S_OK;
}

HRESULT CLevel_ChaosLevel1::Ready_Renderer()
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
void CLevel_ChaosLevel1::Set_CheckGruop()
{
...
void CLevel_ChaosLevel1::Start_Collision()
{
	Set_CheckGruop();
...
void CLevel_ChaosLevel1::Start_Damage()
{
...
void CLevel_ChaosLevel1::Start_QuadTree()
{
...
void CLevel_ChaosLevel1::End_Damage()
{
...
void CLevel_ChaosLevel1::End_Picking()
{
...
void CLevel_ChaosLevel1::End_Collision()
{
...
void CLevel_ChaosLevel1::End_QuadTree()
{
...
CLevel_ChaosLevel1 * CLevel_ChaosLevel1::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CLevel_ChaosLevel1*	pInstance = new CLevel_ChaosLevel1(pDevice, pContext);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("Failed to Created : CLevel_ChaosLevel1");
