...
HRESULT CState_WDR_Attack_3::Initialize()
{
	...
	m_SoundFrames.push_back(SOUNDDESC(0, TEXT("Effect"), TEXT("WDR_11.wav"))); //  Skill
...
void CState_WDR_Attack_3::Tick_State_Control(_float fTimeDelta)
{
	...
	else if (true == m_IsAttackContinue && 110 <= iAnimIndex)
	{
		Vec3 vClickPos;
		if (true == m_pPlayer->Get_CellPickingPos(vClickPos))
			m_pPlayer->Set_TargetPos(vClickPos);
...
void CState_WDR_Attack_3::Tick_State_NoneControl(_float fTimeDelta)
{
	...
	if (-1 != m_AttackFrames[m_iAttackCnt] && m_AttackFrames[m_iAttackCnt] <= m_pPlayer->Get_ModelCom()->Get_Anim_Frame((_uint)m_Attack_3))
	{
		m_iAttackCnt++;
		Effect_Attack();
	}
...
void CState_WDR_Attack_3::Effect_Attack()
{
