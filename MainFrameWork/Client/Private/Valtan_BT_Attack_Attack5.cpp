...
{
	if (m_pGameObject->Get_ModelCom()->Get_CurrAnim() == m_vecAnimDesc[1].iAnimIndex && m_fLoopTime > m_vecAnimDesc[1].fMaxLoopTime && m_iStack < 4)
	{
		m_iStack++;
		m_fLoopTime = 0.f;	
	...
	else if (m_pGameObject->Get_ModelCom()->Get_CurrAnim() == (m_vecAnimDesc[1].iAnimIndex) && m_fLoopTime > m_vecAnimDesc[1].fMaxLoopTime&& m_iStack==4)
	{
