...
CBT_Node::BT_RETURN CValtan_BT_Spawn::OnUpdate(_float fTimeDelta)
{
	...
	{
		m_bSoundOn[0] = true;
		CSound_Manager::GetInstance()->PlaySoundFile(L"Effect", L"Boss_Scream.wav", 0.8f);
		CSound_Manager::GetInstance()->PlayBGM_AddChannel(L"Commander of Beast Valtan", L"BGM", L"Commander of Beast Valtan.wav", 0.3f);
	}
	if (m_pGameObject->Get_ModelCom()->Get_CurrAnim() == m_vecAnimDesc[3].iAnimIndex && m_bSoundOn[1] == false)
	{
		m_bSoundOn[1] = true;
		CSound_Manager::GetInstance()->PlaySoundFile(L"Effect", L"IamCommanderOfBeastValtan.wav", 0.7f);
	}
