#include "stdafx.h"
#include "..\Public\State_WR_Identity_Attack_4.h"
...
CState_WR_Identity_Attack_4::CState_WR_Identity_Attack_4(const wstring& strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Slayer* pOwner)
	: CState(strStateName, pMachine, pController), m_pPlayer(pOwner)
{
}

HRESULT CState_WR_Identity_Attack_4::Initialize()
{
	m_Attack_4 = m_pPlayer->Get_ModelCom()->Initailize_FindAnimation(L"att_identity1_1_04", 2.f);
	if (m_Attack_4 == -1)
		return E_FAIL;

	if (m_pPlayer->Is_Control())
		m_TickFunc = &CState_WR_Identity_Attack_4::Tick_State_Control;
	else
		m_TickFunc = &CState_WR_Identity_Attack_4::Tick_State_NoneControl;
...
void CState_WR_Identity_Attack_4::Enter_State()
{
	m_bEffect = false;
	m_iAttackCnt = 0;
	m_iSoundCnt = 0;
	...
	static_cast<CController_WR*>(m_pController)->Set_Attack_Desc(3);
}

void CState_WR_Identity_Attack_4::Tick_State(_float fTimeDelta)
{
...
void CState_WR_Identity_Attack_4::Tick_State_Control(_float fTimeDelta)
{
...
void CState_WR_Identity_Attack_4::Effect_Rage_Attack_4()
{
	if (!m_bEffect)
	{
		m_bEffect = true;
...
CState_WR_Identity_Attack_4* CState_WR_Identity_Attack_4::Create(wstring strStateName, CStateMachine* pMachine, CPlayer_Controller* pController, CPlayer_Slayer* pOwner)
{
	CState_WR_Identity_Attack_4* pInstance = new CState_WR_Identity_Attack_4(strStateName, pMachine, pController, pOwner);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("Failed To Cloned : CState_WR_Identity_Attack_4");
...
void CState_WR_Identity_Attack_4::Free()
{
	__super::Free();
}
