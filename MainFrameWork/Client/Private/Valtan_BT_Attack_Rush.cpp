...
CBT_Node::BT_RETURN CValtan_BT_Attack_Rush::OnUpdate(const _float& fTimeDelta)
{
	...
	if (m_pGameObject->Get_ModelCom()->Get_CurrAnim() == m_vecAnimDesc[0].iAnimIndex && m_pGameObject->Get_ModelCom()->Get_Anim_Frame(m_vecAnimDesc[0].iAnimIndex) > 48 && m_iLoop < 3)
	{
		...
		m_pEffectWarning = nullptr;
		Add_Sound(L"Effect", L"Voice2", 1.f); 
		Add_Sound(L"Effect", L"WWISEDEFAULTBANK_S_MOB_G_VOLTAN2#288 (673586812)", 1.f);
		CEffect_Manager::EFFECTPIVOTDESC tDesc;
		tDesc.pPivotMatrix = &m_pGameObject->Get_TransformCom()->Get_WorldMatrix();
	...
	{
		Add_Sound(0, 0, L"WWISEDEFAULTBANK_S_MOB_G_VOLTAN2#128 (662482722)", L"Effect", 12);
		if(m_bSoundOn[0]&&m_iLoop== 1)
			Add_Sound(0, 1, L"WWISEDEFAULTBANK_S_MOB_G_VOLTAN2#128 (662482722)", L"Effect", 12);
		if (m_bSoundOn[1] && m_iLoop == 2)
			Add_Sound(0, 2, L"WWISEDEFAULTBANK_S_MOB_G_VOLTAN2#128 (662482722)", L"Effect", 12);
