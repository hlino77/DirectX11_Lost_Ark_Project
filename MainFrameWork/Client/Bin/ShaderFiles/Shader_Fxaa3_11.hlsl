...
{
    return g_FinalTarget.SampleLevel(AnisotropicClampSampler, pos.xy, 0.0);
}

float4 FxaaTexGrad(float2 pos, float2 grad)
{
    return g_FinalTarget.SampleGrad(AnisotropicClampSampler, pos.xy, grad, grad);
}

float4 FxaaTexOff(float2 pos, int2 off)
{
    return g_FinalTarget.SampleLevel(AnisotropicClampSampler, pos.xy, 0.0, off.xy);
}
...
#endif
    float lengthSign = horzSpan ? -gFrame.y : -gFrame.x;
    if (!horzSpan)
        lumaN = lumaW;
    if (!horzSpan)
        lumaS = lumaE;
...
--------------------------------------------------------------*/
    ...
    float2 offNP = horzSpan ?
        FxaaFloat2(gFrame.x, 0.0) :
        FxaaFloat2(0.0f, gFrame.y);
...
--------------------------------------------------*/
    ...
    float subPixelOffset = (0.5 + (dstN * (-1.0 / spanLength))) * lengthSign;
        ...
        float ox = horzSpan ? 0.0 : subPixelOffset*2.0/gFrame.x;
        float oy = horzSpan ? subPixelOffset*2.0/gFrame.y : 0.0;
...
technique11 DefaultTechnique
{
    pass Fxaa3_11 // 0
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN_FXAA();
        GeometryShader = NULL;
        HullShader = NULL;
        DomainShader = NULL;
