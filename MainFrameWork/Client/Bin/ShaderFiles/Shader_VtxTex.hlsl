...
PS_OUT PS_MAIN_MASKED_TEXCOLOR(PS_IN In)
{
	PS_OUT		Out = (PS_OUT)0;

	Out.vColor = g_MaskTexture.Sample(DefaultSampler, In.vTexUV) * g_Color;
...
PS_OUT PS_MAIN_GAMMA(PS_IN In)
{
    PS_OUT Out = (PS_OUT) 0;

    Out.vColor = pow(g_DiffuseTexture.Sample(LinearSampler, In.vTexUV), 2.2f);
...
technique11 DefaultTechnique
{
	...
	pass Gamma	// No. 24
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_MAIN_GAMMA();
    }
}